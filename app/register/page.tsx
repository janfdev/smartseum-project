...
const ERROR_MESSAGES: Record<string, string> = {
  OAuthCallback: "Terjadi kesalahan saat mendaftar. Coba lagi.",
  ...
  const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setRegisterError(data.error || "Gagal mendaftar");
      }
    } catch {
      setRegisterError("Terjadi kesalahan koneksi");
    } finally {
    ...
    <div className="w-full max-w-md mx-auto">
      <div className="flex flex-col items-center mb-10 text-center">
        ...
        <p className="text-black/40 dark:text-white/40 text-sm mt-2 font-light max-w-[280px]">
          Bergabung bersama kami dan nikmati AR interaktif
        </p>
      </div>
          ...
          {emailSent ? (
            <div className="text-center bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-6">
              <div className="w-12 h-12 bg-emerald-500/20 rounded-full flex items-center justify-center mx-auto mb-4">
                <svg className="w-6 h-6 text-emerald-600 dark:text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
              </div>
              <h3 className="text-lg font-bold text-emerald-600 dark:text-emerald-400 mb-2">Cek Email Anda!</h3>
              <p className="text-sm text-black/60 dark:text-white/60">Tautan rahasia untuk melanjutkan pendaftaran telah dikirim ke email Anda. Silakan verifikasi untuk dapat masuk.</p>
            </div>
          ) : (
            <>
              <form onSubmit={handleRegister} className="space-y-4">
                ...
                >
                  {emailLoading ? <Loader2 className="w-5 h-5 animate-spin mx-auto text-white dark:text-black" /> : "Daftar Akun Baru"}
                </button>
              </form>
